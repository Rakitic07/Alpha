...
/**
 * Day change % of the latest bar vs the previous trading session's close —
 * matches the "day change" shown on NSE regardless of the chart timeframe.
 * For intraday timeframes it walks back to the last candle of the prior day;
 * for daily/weekly/monthly it's simply the previous candle's close.
 */
export function dayChangePct(candles: Candle[]): number {
  if (candles.length < 2) return 0;
  const last = candles[candles.length - 1];
  const lastDay = dayKey(last.time);
  for (let i = candles.length - 2; i >= 0; i--) {
    if (dayKey(candles[i].time) !== lastDay) {
      const prevClose = candles[i].close;
      return prevClose ? ((last.close - prevClose) / prevClose) * 100 : 0;
    }
  }
  // Only one day of data present — fall back to the previous candle.
  const prev = candles[candles.length - 2];
  return prev.close ? ((last.close - prev.close) / prev.close) * 100 : 0;
}
...
export interface BreakoutSignal {
  ...
  changePct: number; // % change of last close vs previous close (same timeframe)
  dayChangePct: number; // % change vs previous trading session (NSE day change)
...
export function computeBreakout(candles: Candle[], n = 20): BreakoutSignal {
  const empty: BreakoutSignal = {
    direction: 'none', volRatio: 1, level: null, distancePct: 0, changePct: 0, dayChangePct: 0, score: 0, lastClose: 0, startIndex: -1,
  };
  ...
  const dayChg = dayChangePct(candles);
  const volRatio = volumeRatio(candles, n);
  const dc = donchian(candles, n);

  if (!dc) {
    return { ...empty, volRatio, changePct, dayChangePct: dayChg, lastClose: last.close };
  }
