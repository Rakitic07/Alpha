...
export async function scanBreakouts(symbols: string[], tf: Timeframe): Promise<ScanResult> {
  ...
  await withConcurrency(
    ...
    async (symbol) => {
      ...
      try {
        ...
        rows.push({
          ...
          changePct: breakout.dayChangePct,
    ...
    8,   // concurrency — a touch higher so the list populates faster
    70,  // stagger between workers (ms) — avoid burst
    110, // throttle per request (ms)
  );
