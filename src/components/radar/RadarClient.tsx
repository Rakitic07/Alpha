...
/** Maps an active indicator to its study-library topic. */
const INDICATOR_TOPIC: Record<string, string> = {
  ema9: 'ema',
  ema21: 'ema',
  ema50: 'ema',
  sma200: 'sma200',
...
const CANDLE_INTERVAL_MS = 20_000;
const LIVE_FLUSH_MS = 300;

const LS_INDICATORS = 'radar:indicators';
const LS_TIMEFRAME = 'radar:timeframe';
...
export default function RadarClient() {
  ...
  // Chart indicators + fullscreen + study mode.
  const [indicators, setIndicators] = useState<IndicatorId[]>(['ema21']);
  const [chartFull, setChartFull] = useState(false);
  const [studyMode, setStudyMode] = useState(false);
  const studyTopic = indicators.length ? INDICATOR_TOPIC[indicators[0]] : undefined;
  const toggleIndicator = useCallback((id: IndicatorId) => {
    setIndicators((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
  }, []);

  // Restore + persist user preferences (indicators, timeframe) across refreshes.
  // Hydration-safe: read localStorage only after mount, then keep it in sync.
  const prefsHydrated = useRef(false);
  useEffect(() => {
    try {
      const rawInd = localStorage.getItem(LS_INDICATORS);
      if (rawInd) {
        const arr = JSON.parse(rawInd);
        if (Array.isArray(arr)) setIndicators(arr.filter((x): x is IndicatorId => typeof x === 'string'));
      }
      const rawTf = localStorage.getItem(LS_TIMEFRAME);
      if (rawTf && (TIMEFRAMES as string[]).includes(rawTf)) setTimeframe(rawTf as Timeframe);
    } catch {
  ...
  // Current price marker for the chart: the last (live-folded) candle close,
  // coloured by the live day-change (falling back to the scan's change).
  const currentPrice = displayCandles.length ? displayCandles[displayCandles.length - 1].close : null;
  const currentChangePct =
    typeof liveSelChange === 'number' ? liveSelChange : selectedRow?.changePct ?? null;
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
            ...
            <div className="flex items-center gap-2.5">
              ...
              {candlesLoading && !studyMode && (
                <FontAwesomeIcon icon={faSpinner} className="w-3.5 h-3.5 text-gray-500 animate-spin" />
              )}
              <button
                onClick={() => setStudyMode((v) => !v)}
                title={studyMode ? 'Back to the chart' : 'Study mode — learn each indicator'}
                className={`flex items-center gap-1.5 px-2.5 h-7 rounded-lg border text-[11px] font-semibold transition-colors ${
                  studyMode
                    ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300'
                    : 'border-white/5 bg-zinc-800/40 text-gray-400 hover:text-gray-100'
                }`}
              >
                <FontAwesomeIcon icon={studyMode ? faChartColumn : faBookOpen} className="w-3 h-3" />
                <span className="hidden sm:inline">{studyMode ? 'Chart' : 'Study'}</span>
              </button>
              <IndicatorMenu value={indicators} onToggle={toggleIndicator} />
              <button
                onClick={() => setChartFull(true)}
                title="Expand chart (full screen)"
                className="flex items-center justify-center w-7 h-7 rounded-lg border border-white/5 bg-zinc-800/40 text-gray-400 hover:text-gray-100 hover:bg-zinc-800/70 transition-colors"
              >
          ...
          <div className="flex-1 min-h-0 p-2 sm:p-3">
            {studyMode ? (
              <StudyPanel initialTopicId={studyTopic} />
            ) : candlesData?.error ? (
      ...
      {/* Full-screen chart (≈80% of the viewport) */}
      {chartFull && (
        <div
        ...
        >
          <div
            className="relative flex flex-col w-[80vw] h-[80vh] rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-900/60 to-slate-900 shadow-2xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-white/5">
              <div className="flex items-center gap-3 min-w-0">
                <span className="text-base font-bold text-gray-100 truncate">{selected ?? '—'}</span>
                {typeof liveSelChange === 'number' && (
                  <span className={`text-[13px] font-bold tabular-nums ${liveSelChange >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    {liveSelChange >= 0 ? '+' : ''}
                    {liveSelChange.toFixed(2)}%
                  </span>
                )}
                <div className="hidden sm:flex items-center gap-1 bg-zinc-800/40 border border-white/5 rounded-xl p-1">
                  {TIMEFRAMES.map((tf) => (
                    <button
                      key={tf}
                      onClick={() => setTimeframe(tf)}
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setChartFull(false)}
                  title="Close"
                  className="flex items-center justify-center w-7 h-7 rounded-lg border border-white/5 bg-zinc-800/40 text-gray-400 hover:text-red-300 hover:bg-red-500/10 transition-colors"
                >
            ...
            <div className="flex-1 min-h-0 p-3">
              {studyMode ? (
                ...
                <div className="flex items-center justify-center h-full text-gray-500 text-sm">{candlesData.error}</div>
              ) : (
                <CandlestickChart
                  key={`full-${selected ?? ''}-${timeframe}`}
                  candles={displayCandles}
                  pivots={candlesData?.pivots ?? []}
                  donchianHigh={candlesData?.donchianHigh ?? null}
                  donchianLow={candlesData?.donchianLow ?? null}
                  ...
                  currentChangePct={currentChangePct}
                  indicators={indicators}
                  resetKey={`full-${selected ?? ''}-${timeframe}`}
                />
              )}
            </div>
          </div>
...
// ---------------------------------------------------------------------------
// Indicator picker dropdown (overlays + oscillator panels).
// ---------------------------------------------------------------------------
function IndicatorMenu({ value, onToggle }: { value: IndicatorId[]; onToggle: (id: IndicatorId) => void }) {
  ...
  const panels = INDICATORS.filter((i) => i.kind === 'panel');
  const valueSet = new Set(value);

  const renderItem = (id: IndicatorId, label: string, desc: string, color: string) => {
    const on = valueSet.has(id);
    return (
      <button
        key={id}
        onClick={() => onToggle(id)}
        className={`w-full flex items-start gap-2.5 px-2.5 py-1.5 rounded-lg text-left transition-colors ${
          on ? 'bg-indigo-500/10' : 'hover:bg-white/5'
        }`}
      >
        <span
          className={`mt-0.5 flex items-center justify-center w-4 h-4 rounded border shrink-0 ${
            on ? 'bg-indigo-500 border-indigo-500' : 'border-white/20'
          }`}
        >
          {on && <FontAwesomeIcon icon={faCheck} className="w-2.5 h-2.5 text-white" />}
        </span>
        <span className="min-w-0">
          <span className="flex items-center gap-1.5 text-[12px] font-medium text-gray-200">
            <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: color }} />
            {label}
          </span>
          <span className="block text-[10px] text-gray-500 leading-tight">{desc}</span>
        </span>
      </button>
    );
  };
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {value.length > 0 && (
          <span className="inline-flex items-center justify-center min-w-4 h-4 px-1 rounded-full bg-indigo-500/30 text-[9px] text-indigo-200">
            {value.length}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-72 z-[60] max-h-[70vh] overflow-y-auto rounded-xl border border-white/10 bg-slate-900/95 backdrop-blur-md shadow-2xl p-2">
          <p className="px-2.5 pt-1 pb-1.5 text-[9px] uppercase tracking-wider text-gray-600">Overlays</p>
          {overlays.map((i) => renderItem(i.id, i.label, i.desc, i.color))}
          <div className="my-1.5 border-t border-white/5" />
          <p className="px-2.5 pb-1.5 text-[9px] uppercase tracking-wider text-gray-600">Oscillators</p>
          {panels.map((i) => renderItem(i.id, i.label, i.desc, i.color))}
        </div>
      )}
    </div>
  );
