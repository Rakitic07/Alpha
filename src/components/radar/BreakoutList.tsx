            ...
            <span className="flex items-center gap-1 text-[11px] tabular-nums text-gray-300">
              {live && (
                <span className="relative flex h-1.5 w-1.5" title="Live price">
                  <span className={`absolute inline-flex h-full w-full animate-ping rounded-full ${up ? 'bg-emerald-400/70' : 'bg-red-400/70'}`} />
                  <span className={`relative inline-flex h-1.5 w-1.5 rounded-full ${up ? 'bg-emerald-400' : 'bg-red-400'}`} />
                </span>
              )}
              ₹{fmtPrice(price)}
            </span>
    ...
    <Virtuoso
      ...
      itemContent={(index, row) => (
        <RowItem
          ...
          live={livePrice?.has(row.symbol) ?? false}
          onSelect={onSelect}
          onRemove={onRemove}
