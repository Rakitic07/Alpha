...
interface StatsBarProps {
  stats: ScreenerStats;
  activeTab: 'all' | 'portfolio' | 'others';
  onTabChange: (tab: 'all' | 'portfolio' | 'others') => void;
  filteredCount: number;
}

export default memo(function StatsBar({ stats, activeTab, onTabChange, filteredCount }: StatsBarProps) {
  const { total, portfolioCount, rankedPortfolioCount, rankBuckets, mcapBreakdown } = stats;
  ...
  return (
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
      ...
      {/* Portfolio rank distribution + Market cap breakdown */}
